              ...
              sales.map(s => {
                ...
                return (
                  <tr key={s.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td className="p-2 text-gray-900 dark:text-gray-100 text-right break-words">
            ...
            sales.map(s => {
              ...
              return (
                <div key={s.id} className="bg-white dark:bg-gray-700 rounded-lg shadow p-4 space-y-2 border border-gray-200 dark:border-gray-600">
                  <p className="text-gray-900 dark:text-gray-100"><span className="font-medium">Producto:</span> {producto?.name || 'Producto desconocido'}</p>
                  <p className="text-gray-900 dark:text-gray-100"><span className="font-medium">Cantidad:</span> {s.quantity}</p>
                  <p className="text-gray-900 dark:text-gray-100"><span className="font-medium">Tipo:</span> {s.type === 'sale' ? 'Venta' : 'Desecho'}</p>
                  <p className="text-gray-900 dark:text-gray-100"><span className="font-medium">Fecha:</span> {format(s.date, 'dd/MM/yyyy')}</p>
                </div>
              );
            })
          )}
